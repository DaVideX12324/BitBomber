...
## Gracz BitBomber — snap-ruch na gridzie 64px, kładzenie bomb.
## Obsługuje P1 (WASD + Spacja) i P2 (strzałki + RCtrl).
## Jeśli is_bot=true, skrypt jest ignorowany (logika AI osobno).

# --- Konfiguracja (ustawiana przez arenę przy spawnowaniu) ---
@export var player_id: int = 1        # 1 lub 2
@export var is_bot: bool = false

# --- Stałe ---
const GRID_SIZE: int = 64
const MOVE_SPEED: float = 5.0         # kafelki/sekundę

# --- Zasoby bomb ---
const BOMB_SCENE = preload("res://scenes/objects/bomb.tscn")

# --- Staty gracza (modyfikowane przez power-upy) ---
...
# --- Ruch snap ---
var _grid_pos: Vector2i = Vector2i.ZERO  # pozycja w kafelkach
...
# Kolory fallback per player_id
const FALLBACK_COLORS = {
	1: Color(0.2, 0.6, 1.0),   # niebieski
	2: Color(1.0, 0.4, 0.2),   # pomarańczowy
	3: Color(0.2, 0.9, 0.3),   # zielony
	4: Color(0.9, 0.2, 0.9),   # różowy
}
...
func _ready() -> void:
	...
	# Ustaw pozycję gridu na podstawie pozycji startowej sceny
...
func _handle_movement(delta: float) -> void:
	...
	# Sprawdzenie kolizji pośrednio przez PhysicsLayer (CharacterBody2D i TileMap
	# obsługują to automatycznie przez move_and_collide).
	# Tutaj używamy prostego testu pozycji docelowej:
	var target_pixel = _grid_to_pixel(target_grid)
	# Tymczasowe przesunięcie testowe
	var test_motion = target_pixel - global_position
...
func take_hit() -> void:
	...
	if is_bot:
		...
		# Ludzki gracz — triggeruj last-chance quiz
...
func _respawn() -> void:
	is_alive = true
	# TODO: znajdź bezpieczny kafelek startowy (Arena.get_safe_spawn)
	global_position = _pixel_target
	# Krótka nietykalnōsd (miganie)
	_blink_invincibility(2.0)
...
func _blink_invincibility(duration: float) -> void:
